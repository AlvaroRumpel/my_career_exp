...
export const BADGES: BadgeDef[] = [
  // ---- Inside (11)
  { id: 'aerial-wizard', name: 'Aerial Wizard', group: 'inside', units: b => (twoPm(b) >= 5 ? 1 : 0) + proxy(played(b)) },
  { id: 'float-game', name: 'Float Game', group: 'inside', units: (b, _c, p) => proxy(played(b) && guard(p)) + (twoPm(b) >= 4 ? 0.5 : 0) },
  { id: 'hook-specialist', name: 'Hook Specialist', group: 'inside', units: (b, _c, p) => proxy(played(b)) + (big(p) && twoPm(b) >= 5 ? 0.5 : 0) },
  { id: 'layup-mixmaster', name: 'Layup Mixmaster', group: 'inside', units: b => (twoPm(b) >= 6 ? 1 : 0) + proxy(played(b)) },
  { id: 'paint-prodigy', name: 'Paint Prodigy', group: 'inside', units: (b, _c, p) => (big(p) && twoPm(b) >= 5 ? 1 : 0) + proxy(played(b)) },
  { id: 'physical-finisher', name: 'Physical Finisher', group: 'inside', units: b => (b.fta >= 6 ? 1 : 0) + (twoPm(b) >= 5 ? 0.5 : 0) + proxy(played(b)) },
  { id: 'post-fade-phenom', name: 'Post Fade Phenom', group: 'inside', units: (b, _c, p) => proxy(played(b)) + (big(p) && b.pts >= 20 ? 0.5 : 0) },
  { id: 'post-powerhouse', name: 'Post Powerhouse', group: 'inside', units: (b, _c, p) => proxy(played(b)) + (big(p) && twoPm(b) >= 6 ? 0.5 : 0) },
  { id: 'post-up-poet', name: 'Post-Up Poet', group: 'inside', units: (b, _c, p) => proxy(played(b)) + (big(p) && b.ast >= 3 ? 0.5 : 0) },
  { id: 'posterizer', name: 'Posterizer', group: 'inside', units: b => (twoPm(b) >= 7 ? 1 : 0) + proxy(played(b, 20)) },
  { id: 'rise-up', name: 'Rise Up', group: 'inside', units: (b, _c, p) => (big(p) && twoPm(b) >= 5 ? 1 : 0) + proxy(played(b)) },
  // ---- Outside (5)
  ...
  { id: 'mini-marksman', name: 'Mini Marksman', group: 'outside', units: (b, _c, p) => (guard(p) && b.tpm >= 3 ? 1 : 0) + proxy(played(b)) },
  { id: 'set-shot-specialist', name: 'Set Shot Specialist', group: 'outside', units: b => (b.tpa >= 5 && tpPct(b) >= 0.35 ? 1 : 0) + proxy(played(b)) },
  { id: 'shifty-shooter', name: 'Shifty Shooter', group: 'outside', units: (b, _c, p) => (guard(p) && b.tpm >= 4 ? 1 : 0) + proxy(played(b)) },
  // ---- Playmaking (9)
  { id: 'ankle-assassin', name: 'Ankle Assassin', group: 'playmaking', units: (b, _c, p) => proxy(played(b)) + (b.ast >= 6 ? 0.5 : 0) },
  ...
  { id: 'handles-for-days', name: 'Handles for Days', group: 'playmaking', units: (b, _c, p) => proxy(played(b, 25)) + (b.ast >= 5 ? 0.5 : 0) },
  { id: 'lightning-launch', name: 'Lightning Launch', group: 'playmaking', units: (b, _c, p) => proxy(played(b)) + (twoPm(b) >= 4 && guard(p) ? 0.5 : 0) },
  ...
  { id: 'immovable-enforcer', name: 'Immovable Enforcer', group: 'defense', units: (b, _c, p) => proxy(played(b)) + (b.blk >= 1 ? 0.5 : 0) },
  ...
  { id: 'on-ball-menace', name: 'On-Ball Menace', group: 'defense', units: (b, _c, p) => proxy(played(b, 20)) + (b.stl >= 2 ? 1 : 0) },
  { id: 'paint-patroller', name: 'Paint Patroller', group: 'defense', units: (b, _c, p) => (big(p) ? b.blk : b.blk * 0.5) + proxy(played(b)) },
  { id: 'pick-dodger', name: 'Pick Dodger', group: 'defense', units: (b, _c, p) => proxy(played(b, 20)) + (b.plusMinus >= 8 ? 0.5 : 0) },
  { id: 'post-lockdown', name: 'Post Lockdown', group: 'defense', units: (b, _c, p) => proxy(played(b)) + (big(p) && b.blk >= 2 ? 1 : 0) },
  // ---- Rebounding (2)
  ...
  { id: 'brick-wall', name: 'Brick Wall', group: 'general', units: (b, _c, p) => proxy(played(b, 20)) + (big(p) && b.plusMinus >= 5 ? 0.5 : 0) },
  { id: 'slippery-off-ball', name: 'Slippery Off-Ball', group: 'general', units: (b, _c, p) => proxy(played(b, 20)) + (b.tpm >= 3 ? 0.5 : 0) },
